#!/bin/bash
set -euo pipefail
SGLANG_LOG="/proj/assert-berzelius/users/x_tuhan/garage/tiny_nla/logs/sglang_rl_16789833.log"
...
while true; do
    ...
    if [ "$cur_count" -gt "$last_count" ]; then
        ...
        if [ -n "$target_step" ]; then
            echo "[$(date)] Step $target_step detected (count=$cur_count). Snapshotting..."
            sleep 10
            ...
            echo "[$(date)] Snapshot step_${target_step} saved."
        fi
    fi
    STATUS=$(squeue -u x_tuhan -h -o "%T" -j 16789833 2>/dev/null | head -1)
